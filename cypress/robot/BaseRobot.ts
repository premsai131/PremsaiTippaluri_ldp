...
export abstract class BaseEyes {
  ...
  //custom method only for this project
  seesValueWithId(id: string,value :string){
  ...
  seesClassVisibile(clas: string) {
    cy.get(`.${clas}`).should('be.visible');
     return this;
  }
...
export class BaseHands {
  ...
  clickOnDomElementWithTarget(dom:string){
    cy.get(dom).invoke('removeAttr','target').click()
    return this
  }
  ...
  typeTextonId(id: string, text: any) {
